<main class="main can-be--dark-dark" role="main">
	<style type="text/css">
		.header-swath--with-image {
			...
			background-image: url(<?= $judul_image ?>);
		}
	...
	<?php
	// Ubah format tanggal menjadi nama bulan dalam Bahasa Indonesia
	$bulan = [
		1 => 'Januari',
		2 => 'Februari',
		3 => 'Maret',
	...
	// Ambil tanggal dari data berita
	$tanggal_berita = $berita_detail['tanggal']; // Misal '2024-10-15'

	// Ubah menjadi format yang diinginkan
	$timestamp = strtotime($tanggal_berita);
	$hari = date('d', $timestamp); // Ambil hari
	$bulan_nama = $bulan[date('n', $timestamp)]; // Ambil nama bulan
	$tahun = date('Y', $timestamp); // Ambil tahun

	// Gabungkan menjadi format yang diinginkan
	$tanggal_format = "$hari $bulan_nama $tahun";
	?>
		...
		<div class="flex-container cf">
			<div class="shift-left--fluid column__primary bg--white can-be--dark-light ">
				<div class="pad--primary breadcrumbs-container">
					<nav class="breadcrumbs" role="navigation">
						<ul class="breadcrumbs__list">
							...
							<li class="breadcrumbs__list-item font--secondary--xs upper dib"><a
									href="<?= base_url('') ?>berita-misi/anak-anak">Anak-anak</a></li>
							<li class="breadcrumbs__list-item font--secondary--xs upper dib"><a
				...
				<div class="pad--primary spacing text page_content pad--btm">
					<h3>Sabat ke <?= $sabat_ke ?> - Triwulan ke <?= $triwulan_ke ?>, <?= $tahun ?></h3>
					<img class="img-story" src="<?= $judul_image ?>" loading="lazy" alt="<?= $story_by ?>, dari <?= $from ?>">
                    <div>
						<h1 class="content"><?= $judul ?></h1>
					</div>

					<div class="container1 berita-detail">
                        <div class="sidebar-c sidebar-ctop">
                            <h3>Tip Cerita</h3>
                            <?= $berita_detail['story_tips']; ?>
                            <h3>Fakta Singkat</h3>
                            <?= $berita_detail['fakta_singkat']; ?>
                        </div>

                        <div class="content1">
                            <p>Kepada guru Sekolah Sabat: Cerita ini untuk hari Sabat, <?= $tanggal_format; ?>.
                            </p><br>
                            <p><span class="author">Oleh Andrew McChesney</span>
                            </p><br>

                            <p><em>
                                <?= $berita_detail['pendahuluan_teks_cerita']; ?>
                            </em></p><br>
                            
                            <div class="content">
                                <p><?= $berita_detail['teks_cerita']; ?></p>
                                <br>
                                <div class="container1">
                                <?php if (!empty($berita_detail['file_pdf'])): ?>
                                    <a href="<?= $berita_detail['file_pdf']; ?>" target="_blank" class="btn btn-primary">Download PDF</a>
                                <?php endif; ?>
                                <?php if (!empty($berita_detail['file_ppt'])): ?>
                                    <a href="<?= $berita_detail['file_ppt']; ?>" target="_blank" class="btn btn-primary">Download PPTX</a>
                                <?php endif; ?>
                                </div>
